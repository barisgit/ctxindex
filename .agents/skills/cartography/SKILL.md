---
name: cartography
description: "Maintain hierarchical codemap.md files for a repo. Use after any
  non-trivial set of changes (refactor, new module, file moves, deletions) to
  refresh affected codemaps, when starting on an unfamiliar codebase, or when
  asked to explain/map repo structure. Incremental: only updates folders whose
  hashes changed."
assets:
...
```bash
python3 ~/.agents/skills/cartography/scripts/cartographer.py init \
  --root ./ \
  --include "src/**/*.ts" \
...
```bash
python3 ~/.agents/skills/cartography/scripts/cartographer.py changes \
  --root ./
```
...
```bash
python3 ~/.agents/skills/cartography/scripts/cartographer.py update \
  --root ./
```
...
|-----------|------------------------|--------------|
| `src/agents/` | Defines agent personalities (Orchestrator, Explorer) and manages model routing. | `src/agents/codemap.md` |
| `src/features/` | Core logic for tmux integration, background task spawning, and session state. | `src/features/codemap.md` |
| `src/config/` | Implements the configuration loading pipeline and environment variable injection. | `src/config/codemap.md` |
```
