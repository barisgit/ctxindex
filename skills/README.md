...
The running CLI is authoritative. Use `ctxindex --help` for the current command surface, `ctxindex describe` and exact-definition JSON output for loaded vocabulary, `ctxindex extension list` for active Extensions, `ctxindex skills list` to discover bundled orientation, and `ctxindex skills get <name>` to read one skill.
