...
Active `prototype-local-daemon-orpc` proposal, design, evaluation, implementation, tasks, seven delta specs, and implemented daemon/RPC/client evidence supplement every affected row below. `promote-local-daemon-architecture` records the remaining gates before that prototype can become the normal stateful runtime; canonical capability specs still own shipped behavior.
