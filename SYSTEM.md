...
>
> **Sources consulted:** `README.md`; `CONTEXT.md`; all 18 canonical capability specs and all 19 sidecars present on 2026-07-18 (`core-model` has none); the `agent-orientation-guidance`, `add-git-extension-catalogs`, `add-threaded-reply-drafts`, and `add-docs-web-surface` delta specs; all four delta specs in `separate-sync-warning-error-accounting`; the `add-threaded-reply-drafts` and `add-docs-web-surface` implementation artifacts; decisions D1–D22 in `docs/design/2026-07-13-context-access-layer.md`; `.agents/skills/repo-development/SKILL.md`; and current CLI help/registry output. Section 13 is the full index.
