...
>
> **Sources consulted:** `README.md`; `CONTEXT.md`; all canonical capability specs and implementation sidecars present on 2026-07-20, including `core-model/implementation.md`; all eight delta specs plus the proposal, design, and implementation artifacts in `redesign-extension-sdk`; the `agent-orientation-guidance`, `add-git-extension-catalogs`, `add-threaded-reply-drafts`, `add-docs-web-surface`, and `separate-sync-warning-error-accounting` changes; decisions D1–D22 in `docs/design/2026-07-13-context-access-layer.md`; `.agents/skills/repo-development/SKILL.md`; and current CLI help/registry output. Section 13 is the full index.
