...
>
> **Sources consulted:** `README.md`; `CONTEXT.md`; all 18 capability specs and all 18 sidecars present on 2026-07-19 (`core-model` has none); the `agent-orientation-guidance`, `add-git-extension-catalogs`, `add-threaded-reply-drafts`, and `add-draft-attachments` delta specs; all four delta specs in `separate-sync-warning-error-accounting`; the implementation artifacts for threaded replies and Draft attachments; decisions D1–D22 in `docs/design/2026-07-13-context-access-layer.md`; `.agents/skills/repo-development/SKILL.md`; and current CLI help/registry output. Section 13 is the full index.
