...
>
> **Sources consulted:** `README.md`; `CONTEXT.md`; all canonical capability specs and implementation sidecars present on 2026-07-20, including `core-model/implementation.md` and `extension-documentation/implementation.md`; the active changes listed in Section 13; decisions D1–D22 in `docs/design/2026-07-13-context-access-layer.md`; `.agents/skills/repo-development/SKILL.md`; current CLI help/registry output; and the active `prototype-local-daemon-orpc` artifacts and implementation evidence. The daemon change is consulted only as disposable, unpromoted prototype evidence; canonical capability specs still win.
...
Active `prototype-local-daemon-orpc` proposal, design, evaluation, implementation, tasks, seven delta specs, and implemented daemon/RPC/client evidence supplement every affected row below as disposable, unpromoted prototype evidence.
