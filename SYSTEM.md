...
>
> **Sources consulted for this refresh:** `CONTEXT.md`; the current `cli-surface`, `documentation-consumption`, `extension-documentation`, `extension-installation`, `extension-catalogs`, `extension-loading`, and `sync-operations` capability specs; their present implementation sidecars, including the new `daemon-operation-streams` sidecar; the active `stream-daemon-operations`, `unify-cli-command-model`, and `rebuild-product-extension-sdk-docs` implementation artifacts; and affected CLI, core, RPC, daemon, web, and Extension codemaps. Canonical capability specs take precedence over older pre-alpha interface listings. Section 13 retains the full source map.
