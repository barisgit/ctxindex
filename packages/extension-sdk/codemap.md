...
- Capability-gated Strategy contracts and callback/event contexts keep provider implementations independent of core persistence and orchestration; `ActionContext.resolveResource` exposes a read-only, selected-Source lookup without exposing storage internals.
...
2. Core supplies `SyncContext`, `SearchContext`, `RetrieveContext`, `DownloadContext`, or `ActionContext` to an Adapter operation; Action handlers may resolve a canonical Ref to an `ActionResource` in the selected Source.
