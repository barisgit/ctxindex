...
4. Operations return Resources/results or use callbacks to emit sync resources, removals, checkpoints, warnings, artifacts, and byte chunks. Action handlers can resolve selected-Source local Resources and cached Artifacts before deriving a provider mutation.
...
- Authored against by `packages/profiles/src/`, `packages/adapters/src/`, external extension packages such as `examples/tenders-extension/extension.ts`, and package-backed Catalog author packages.
