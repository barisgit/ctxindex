...
- `operations.ts`: provides the provider-operation contexts and value contracts for sync emissions, remote search, retrieval, artifact download, and Actions. `ActionContext.resolveResource` offers a read-only selected-Source Resource lookup, while `resolveArtifact` exposes safe descriptor metadata and copied integrity-verified bytes without storage paths or provider access.
