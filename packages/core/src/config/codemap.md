...
- `env-loader.ts` is the central typed environment allowlist. `EnvSchema` includes runtime paths/logging, provider-neutral OAuth routing, Google and Microsoft add-time registration keys, loopback-only Gmail/Calendar/Graph test bases, and test controls; memoized `getEnv()` snapshots `process.env`, while `readEnvironmentVariable()` accepts only Provider-declared names matching `^[A-Z_][A-Z0-9_]*$`.
- Built-in Provider registration inputs are `CTXINDEX_GOOGLE_CLIENT_ID`, optional `CTXINDEX_GOOGLE_CLIENT_SECRET`, and `CTXINDEX_MICROSOFT_CLIENT_ID`; they are read only by local OAuth App add-from-env. Refresh tokens are private Grant secrets, not environment-schema inputs.
