...
- `env-loader.ts` is the central typed environment allowlist. `EnvSchema` includes runtime paths/logging, provider-neutral OAuth routing, Google and Microsoft add-time client keys, loopback-only Gmail/Calendar/Graph test bases, and test controls; memoized `getEnv()` snapshots `process.env`, while `readEnvironmentVariable()` exposes only well-formed provider-declared `CTXINDEX_*` names.
- Built-in OAuth client inputs are `CTXINDEX_GOOGLE_CLIENT_ID`, optional `CTXINDEX_GOOGLE_CLIENT_SECRET`, and `CTXINDEX_MICROSOFT_CLIENT_ID`; refresh tokens are runtime Grant secrets, not environment-schema inputs. `CTXINDEX_GRAPH_MOCK_BASE_URL` remains test-only, and the Adapter constrains mock routing to non-production `127.0.0.1` origins.
...
3. Environment consumers call `getEnv()` for one immutable process snapshot; `client add --from-env` reads provider-declared client credential keys once before persisting secret refs, and `resolveEnvUri()` maps an `env:` URI to that snapshot.
...
- Exported by `index.ts`, the `@ctxindex/core/config` subpath, and `packages/core/src/index.ts`; CLI client import, definitions, paths, logging, secrets, storage, and built-in Adapters consume this seam.
