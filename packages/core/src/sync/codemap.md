...
3. The drive emits through `parseSyncEmission()`; warnings are collected, checkpoints advance the prospective cursor, and Resource refs are checked against the requested Source.
...
- `packages/core/src/source/sync-source.ts` is the Adapter-facing entry; `apps/cli/src/sync/runner.ts` invokes it and consumes `SyncRunResult`.
