...
Implements provider-neutral domain and application services: exported-value Extension collection/complete registration, trusted Git Catalog provenance, immutable direct package installation, OAuth App/Account/private Grant layering, Realm/Source lifecycle, constrained provider operations, generic storage, sync/search/retrieval, Actions, exports, Artifacts, and runtime infrastructure.
...
2. Extension loading resolves manifest-owned entries from built-ins, explicit packages, offline installed Catalog provenance, and verified immutable direct pins, collects exported roots, and validates one complete candidate registry. `oauth-app/` merges Extension Apps with secret-backed local BYOA records; `account/` maintains globally labeled identities; `auth/` requests all loaded same-provider scopes and updates one stable self-sufficient Grant per Account.
...
Public through `src/index.ts` and package subpaths; consumed primarily by `apps/cli/src/deps.ts` and Adapter operation hosts. Detailed capability maps live in child folders including `catalog/`, `direct-extension/`, `extension/`, `registry/`, `oauth-app/`, `account/`, `auth/`, `schema/`, and `source/`.
