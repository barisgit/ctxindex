...
Provides typed opaque secret references, encrypted-file and OS-Keychain backends, no-fallback runtime routing, fresh-install selection, and crash-safe switching of Grant credential references.
...
- `KeychainBackend` adapts `keytar`, maintains a reserved index for listing, performs a write/read/delete availability probe, and refuses native Keychain access in non-live tests unless `CTXINDEX_KEYTAR_MOCK_FILE` is configured.
...
2. File reads derive the envelope-recorded key mode and decrypt `secrets.box`; writes preserve that mode, encrypt all scoped records with a fresh nonce, and atomically replace the file. Keychain operations map scopes to `ctxindex/<scope>` and maintain their index entry.
3. On a missing config, `initializeSecretBackend()` probes Keychain first, probes/prepares file storage only if needed, then atomically persists the selected backend; existing configs are never reselected.
4. A backend switch validates source references, copies and verifies target values, transactionally rewrites Grant refs, commits config, and only then attempts bounded source cleanup. Pre-commit failures leave source or mixed typed refs readable, retries converge, and post-commit cleanup failures become bounded warnings rather than rollback.
...
- `packages/core/src/auth/` consumes `SecretVault`; `apps/cli/src/deps.ts` wires vault/manager instances, and `apps/cli/src/commands/secrets.ts` invokes status/switch behavior without accepting secret values.
