...
2. File reads derive the envelope-recorded key mode and decrypt `secrets.box`; writes preserve that mode, encrypt all scoped records with a fresh nonce, and atomically replace the file. Keychain operations map scopes to `ctxindex/<scope>`; writes publish inventory before the credential and attempt compensation after a reported value-write failure, retaining the primary error and returning no reference if compensation also fails. Deletes are safe to repeat and retain a discoverable stale entry when index cleanup fails.
