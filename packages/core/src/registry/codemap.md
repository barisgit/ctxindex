...
1. The extension loader or legacy constructor provides complete Extension roots. `buildCompleteCandidateRegistry()` validates roots, coalesces equal IDs with every provenance record, visits exact explicitly listed leaves, and follows Provider/Profile values referenced by already collected Adapters, Actions, and OAuth Apps without traversing modules or package dependencies.
