...
4. Install separately validates execution trust and persisted record/manifest/source identity, then delegates package manifest entry resolution, exported-root collection, exact Extension-id selection, and selected-root documentation resolution to the source-neutral Extension loader. Arbitrary package evaluation causes are discarded at that boundary; the selected candidate is validated with active roots and local OAuth App identities through the complete candidate registry before installed provenance changes.
