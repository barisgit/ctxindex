...
- Remote search post-filters and Ref-deduplicates one provider origin, materializes it through one atomic Resource batch, passes through the Adapter's opaque continuation, yields after synchronous storage waits so scheduled cancellation wins, and degrades only exhausted optional-cache contention to a safe `storage_busy` warning. Exact retrieval rechecks cancellation after Adapter completion and immediately before persistence.
