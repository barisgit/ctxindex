...
3. Each installed record derives its immutable snapshot location, validates manifest and source identity, imports the definition, and records Catalog/repository/commit provenance.
...
- Depends on `@ctxindex/extension-sdk`, `packages/core/src/config/`, `packages/core/src/catalog/`, and `packages/core/src/registry/`.
