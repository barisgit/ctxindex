...
- `ArtifactStore` in `artifact-store.ts` implements a SHA-256 content-addressed store under `dataDir()/artifacts`, with write/commit/abort semantics, immutable Ref-to-content binding, and integrity checks on reads; `read()` rejects an optional maximum-size violation from metadata before filesystem access, then returns copied verified bytes without transferring store ownership.
...
2. `resolveCached()` requires a complete, non-deleted same-Source origin Resource, revalidates current descriptor membership and cached metadata, propagates an optional remaining byte budget to the store, then returns safe descriptor fields plus copied exact bytes to an Action; a miss or over-budget rejection performs no authentication, provider I/O, or download.
