...
2. `resolveCached()` revalidates same-Source current descriptor membership and cached metadata, propagates an optional remaining byte budget to the store, then returns safe descriptor fields plus copied exact bytes to an Action; a miss or over-budget rejection performs no authentication, provider I/O, or download.
