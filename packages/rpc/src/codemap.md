...
- `schemas.ts` is the authoritative strict Zod boundary for protocol/runtime identity, inputs, outputs, failures, and count-only sync events.
...
3. Unary procedures unwrap one `RpcResult`; streamed sync unwraps an application iterator whose terminal value is an `RpcResult`.
