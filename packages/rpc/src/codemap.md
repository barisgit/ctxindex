...
- `schemas.ts` is the authoritative strict Zod boundary for protocol/runtime identity, inputs, outputs, failures, count-only sync events, and bounded source-aware Action describe/run values.
...
3. Unary procedures, including Action describe/run, unwrap one `RpcResult`; streamed sync unwraps an application iterator whose terminal value is an `RpcResult`.
