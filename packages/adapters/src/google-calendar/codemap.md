...
- `event.ts` validates Google event variants, strips markup to visibility-safe text, normalizes timed/all-day timing and valid provider zones to canonical IANA names alongside participant/recurrence/provider metadata, emits exact Source-scoped event and series Refs, and returns bounded deterministic warnings for safely skippable variants.
