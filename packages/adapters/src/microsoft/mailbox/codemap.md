...
1. `microsoftMailboxSearchRemote()` validates limits/time bounds plus sender and boolean `unread` filters, builds bounded Graph KQL `$search` (using `*` for query-less filtered enumeration), and requests immutable IDs. It follows at most three validated message pages, excludes drafts/duplicates/out-of-range results, and returns up to 50 normalized resources. When validated provider data remains, it returns a versioned base64url continuation containing the exact normalized query, requested limit, validated progression, and bounded seen IDs plus a truncation warning; resume rejects malformed or query/limit-mismatched tokens before provider I/O and replays a partially consumed page so eligible messages are not lost.
