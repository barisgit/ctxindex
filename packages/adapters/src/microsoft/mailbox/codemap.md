...
- `draft.ts` validates strict standalone/reply Action inputs and Graph recipient syntax. Standalone create/update uses one generic message POST/PATCH; reply create uses one MIME `createReply`, while reply update proves the immutable local parent before one PATCH. Reply responses must match the derived recipient, subject, body, and conversation; every path has no send handler, provider read, retry, or follow-up mutation.
...
5. `microsoftDraftCreate()` maps standalone content to generic create or a locally derived reply to native `createReply`; `microsoftDraftUpdate()` validates the same-Source Draft Ref and, for replies, the stored immutable parent Ref before one Graph patch. Both require immutable-ID preferences, reject non-Draft or mismatched responses, and return the canonical complete Draft Resource for core to materialize without sending.
