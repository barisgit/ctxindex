...
- `draft.ts` validates strict standalone/reply Action inputs, Graph recipient syntax, and every locally derived MIME header against CR/LF injection. Standalone create/update uses one generic message POST/PATCH; reply create uses one MIME `createReply`, while reply update proves the immutable local parent before one PATCH. Stored reply Drafts cannot use the standalone update shape; reply responses must match the derived recipient, subject, line-ending-normalized body, and conversation. Every path has no send handler, provider read, retry, or follow-up mutation.
