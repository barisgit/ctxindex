...
- `event.ts` validates permissive Graph event DTOs; resolves Graph IANA/Windows zones to canonical IANA names for instant conversion and display metadata; preserves recurring-occurrence series identity, including Graph occurrences that omit `originalStart`; and normalizes timing, all-day dates, participants, response/status, recurrence, links, locations, HTML/plain-text descriptions, timestamps, immutable event Refs, removals, and warning-bearing malformed records into the provider-neutral calendar schema.
