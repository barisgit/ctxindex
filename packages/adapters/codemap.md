...
Packages ctxindex's built-in provider integration layer, exposing production Adapter, OAuth provider, and Extension definitions—including Gmail and Microsoft Graph Outlook Draft create/update handlers—from `packages/adapters/src/` as the private workspace package `@ctxindex/adapters`.
...
3. Adapter code translates Google, Microsoft Graph, or filesystem data into Profile-shaped resources, streamed emissions, Artifacts, warnings, checkpoints, or Action results for core to validate and persist. Draft implementations consume the shared communication Action schemas, issue exactly one provider create/update mutation, and return a canonical Draft Resource; no send Action is registered.
