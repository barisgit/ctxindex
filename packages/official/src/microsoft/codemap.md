# packages/official/src/microsoft/
...
- Exported from `packages/official/src/index.ts` as `microsoftOAuthProvider`, `ctxindexMicrosoftOAuthApp`, and Calendar/mailbox config schemas and Adapter definitions; the App and both Adapters are registered in `builtins.ts`.
