...
- `flags.ts` centralizes `parseFlags`, `hasHelpFlag`, `stringFlag`, and `listFlag`; specialized parsers add command-specific validation.
...
- Consumed by matching modules under `apps/cli/src/commands/`, plus `auth/handle-auth-command.ts`, `source/handle-source-command.ts`, and `sync/runner.ts`.
