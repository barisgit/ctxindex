...
- `search.ts` is the single declaration of Search positionals and options. Realm, Source, and field filters are repeatable Citty strings; routing flags remain typed booleans and structured output resolves through the shared pretty/text/json contract.
- Search normalization validates dates, counts, typed `name=value` fields, query-less filtering, routing conflicts, local-offset versus remote-continuation constraints, and the text-only `--refs` projection before orchestration starts.
