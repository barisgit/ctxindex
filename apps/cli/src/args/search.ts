import type { ExecuteSearchInput } from '@ctxindex/core/search'
...
export const searchUsage =
  'search <query> [--realm <slug>] [--provider <id>] [--adapter <id>] [--source <id>] [--mime <pattern>] [--kind <kind>] [--since <iso>] [--until <iso>] [--include-deleted] [--limit <n>] [--explain] [--json]'
...
function parseLimit(
  ...
  return Number.isFinite(parsed) && parsed >= 1
    ? { ok: true, value: parsed }
    : { ok: false, message: `search: invalid --limit: ${value}` }
}
...
export function parseSearchArgs(args: string[]): SearchArgs {
  ...
  const providerFilter =
    stringFlag(flags, 'provider') ?? stringFlag(flags, 'adapter')
