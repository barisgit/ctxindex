...
- `main.ts` is the composition root and command registry; `rootCommand` binds the descriptors from `commands/` and `runCli` adapts citty's process behavior to a returned numeric exit code.
- `args/` is a pure parsing layer built around discriminated unions; `commands/` contains thin command adapters; `format/` contains presentation and error-to-exit adapters.
...
- `definitions.ts` centralizes config and extension loading through `loadCliDefinitions`; `auth/`, `source/`, and `sync/` isolate multi-step workflows, while `skills/` abstracts filesystem versus embedded skill content.
...
5. Help output additionally calls `definitions.ts#loadCliDefinitions`, emits redacted extension diagnostics, and prints the loaded registry interface.
...
- Detailed submaps: `apps/cli/src/args/codemap.md`, `commands/codemap.md`, `format/codemap.md`, `auth/codemap.md`, `source/codemap.md`, `sync/codemap.md`, and `skills/codemap.md`.
