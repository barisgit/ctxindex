...
2. Database-backed flows require explicit `init`; `client add --from-env` validates the loaded provider, checks initialization before reading declared environment names, then persists typed secret refs and metadata.
