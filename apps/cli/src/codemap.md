...
3. `account add --app` opens one retained direct dependency lifetime, resolves one exact same-provider App from its complete inventory, and creates or updates one stable Grant with a private App-config snapshot; removal clears bound Source grants.
