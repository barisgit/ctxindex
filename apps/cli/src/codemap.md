...
6. `docs list|get|search` composes the build-time bundled product-documentation source with the loaded Extension documentation projection, keeps Markdown inert, requires explicit output for assets, and performs bounded deterministic in-memory queries without web runtime or network lookup.
