...
- Detached launch uses a strict infrastructure allowlist plus an exact Provider mock-endpoint allowlist only under `NODE_ENV=test`; Provider and arbitrary Extension credential variables never enter the long-lived daemon environment. Account stream settlement or rejection always aborts interactive prompt input before cleanup.
