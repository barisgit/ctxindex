...
- `service.ts` selects the runtime once. Direct mode composes the bundled source with the projection from `loadCliDefinitions()`; selected-daemon mode keeps bundled docs local and delegates only Extension list/get/search through the typed RPC client, with no direct-load fallback after selection.
...
2. A `docs list`, `docs get`, or `docs search` invocation selects its route once. With no daemon it loads current Extension definitions, prints host-generated diagnostics, and constructs one combined core service. With an exact daemon selector it keeps the embedded bundled service local and obtains Extension results only from the daemon's immutable projection.
