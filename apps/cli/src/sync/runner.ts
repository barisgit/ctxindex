...
function failedSource(sourceId: string, error: unknown): FailedSourceSync {
  ...
  return {
    ...
    error: {
      code,
      message: `Sync failed for Source "${sourceId}" (${code})`,
    },
    exitCode: mapErrorToExit(error),
  }
}

function warningsFor(result: CompletedSourceSync): SyncWarningOutput[] {
  return result.run.warnings.map((warning) => ({
  ...
  const lines = output.results.map((result) => {
    if (result.status === 'failed') {
      return format === 'compact'
        ? `${result.sourceId} failed code=${result.error.code} exit=${result.exitCode}`
        : `${result.sourceId}\tfailed\tcode=${result.error.code}\texit=${result.exitCode}`
    }
    const run = result.run
    return format === 'compact'
      ? `${result.sourceId} completed +${run.added} ~${run.updated} -${run.deleted} !${run.errorsCount}`
      : `${result.sourceId}\tcompleted\tadded=${run.added}\tupdated=${run.updated}\tdeleted=${run.deleted}\terrors=${run.errorsCount}`
  })
...
export async function handleSyncCommand(
  ...
  try {
    ...
    const completed = results.filter(
      (result): result is CompletedSourceSync => result.status === 'completed',
    )
    const output: SyncOutput = {
      mode: parsed.mode,
      results,
      warnings: completed.flatMap(warningsFor),
    }
