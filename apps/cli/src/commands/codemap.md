...
- `secrets.ts` validates its closed backend target locally, then ensures and uses the daemon's typed status/set procedures on supported platforms without direct fallback; unsupported platforms retain the leased direct manager. `status.ts`, `get.ts`, `export.ts`, and `skills.ts` define their full public grammar and pass typed values directly into focused workflows. `export.ts` preserves exact stdout bytes while ensuring the daemon and consuming its opaque local transfer ticket; only unsupported platforms retain direct composition. Ref and secret-backend semantic validation remains effect-free inside those workflows, while enum/default/help behavior comes from the descriptors.
