...
- `account`, `oauth-app`, `realm`, `source`, `extension list`, `status`, `search`, and `get` reuse the shared pretty/text/json definition and resolve destination-aware defaults before workflow effects. `export` and `describe` retain their independent format domains, while sync retains its existing modes pending streaming integration.
