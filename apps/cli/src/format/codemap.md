...
- `output.ts` owns `pretty|text|json` selection, the shared `-f`/`--format` Citty definition, TTY/non-TTY defaults, compact JSON, escaped TSV with reserved `\N` nulls, grapheme-safe display wrapping, and shared collection presentation. Pretty collections use horizontal tables only when complete values fit, bounded vertical tables when structurally possible, and plain labeled cards below that minimum; no path ellipsizes values.
...
- `thread.ts` renders complete flattened Resource rows plus tree depth for pretty/text while preserving the typed tree envelope in JSON; `artifact.ts` uses the same collection contract for descriptor listing.
