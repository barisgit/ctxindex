...
- Catalog formatters render persisted Git snapshots, package-or-literal entry replay provenance, versionless Marketplace results, trusted build results, and curation-backed managed install records in text or JSON; acquisition timestamps remain intact and derived non-negative snapshot age is surfaced at formatting time. Registry formatters expose unified direct-or-Catalog provenance and unavailable managed records. Direct formatters retain credential-free targets, exact immutable provenance with install/update times, and forced-removal data-preservation semantics.
