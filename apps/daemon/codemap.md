...
2. `startDaemon()` canonicalizes runtime identity and endpoint paths, acquires exclusive lifecycle and database leases, publishes `starting` metadata, loads persisted Extensions without Catalog/network acquisition, opens and migrates SQLite, and composes core services.
