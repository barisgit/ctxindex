...
User argv enters `apps/cli/bin/ctxindex.mjs`, flows to `apps/cli/src/main.ts#runCli`, then through citty command descriptors, typed parsers, dependency composition, core service calls, and output/exit adapters. Trusted Git Catalog add/refresh explicitly acquire immutable snapshots while installation and startup stay offline. Initialization selects and persists an available secret backend for a fresh config; the light `secrets` surface reports safe status or requests a crash-safe backend switch. OAuth onboarding follows the explicit `client` -> `account` -> `source` layering; the removed `auth` command has no alias. The application returns a numeric status to the executable shim.
