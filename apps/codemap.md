...
User argv enters `apps/cli/bin/ctxindex.mjs`, flows to `apps/cli/src/main.ts#runCli`, then through citty command descriptors, typed parsers, dependency composition, core service calls, and output/exit adapters. Initialization selects and persists an available secret backend for a fresh config; the light `secrets` surface reports safe status or requests a crash-safe backend switch. The application returns a numeric status to the executable shim.
