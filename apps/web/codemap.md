...
- The landing page leads with the shell-capable-agent contract and a copyable providerless demo. `components/demo-quickstart.tsx` centralizes the replaceable launch commands/result, `copy-button.tsx` owns the client-side copy interaction, and `demo-video.tsx` activates media only when both a source and captions exist. Supporting agent, trust-boundary, and Extension sections use flat divider-led structures instead of repeated marketing-card grids or decorative imagery.
- `app/opengraph-image.tsx` renders the homepage social image from the canonical mark geometry and landing claim; root layout metadata exposes it only through an absolute configured site origin.
...
6. The homepage moves from the agent-facing claim into one providerless Extension install/sync/search path, then projects agent usage, provider canonicality, trusted Extension authoring, and optional captioned video without adding runtime coupling.
7. `/design` applies forced light and dark token scopes to representative typography, controls, code, and logo sizes without introducing a parallel theme implementation.
8. Focused homepage and content-contract tests enforce first-viewport claims, centralized demo placeholders, task-oriented navigation, checked Extension-example links, and authored internal-route resolution before MDX type generation and production build.
