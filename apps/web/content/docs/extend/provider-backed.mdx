...
The tested [`examples/issues-extension`](https://github.com/barisgit/ctxindex/tree/main/examples/issues-extension) package is a compact complete graph. Its endpoints use the reserved `.invalid` domain and its public client id is a non-secret fixture.
...
Use the [complete checked source](https://github.com/barisgit/ctxindex/blob/main/examples/issues-extension/extension.ts) when copying: it validates Source config and provider JSON, propagates cancellation, encodes paths, and returns typed result envelopes.
