...
The standalone [`barisgit/ctxindex-extensions`](https://github.com/barisgit/ctxindex-extensions) repository is the checked provider-backed reference: it defines a credential-free public GitHub Provider, Profile, indexed Adapter, Extension, Catalog, documentation, and synthetic HTTP tests using only published ctxindex packages. Start there for a runnable implementation. The OAuth example below adds registration policy to the same graph shape.
