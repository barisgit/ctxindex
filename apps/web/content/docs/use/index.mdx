...
Use the compact default for orientation and request `describe --full --json` only when you need the complete registry snapshot.

## Materialize deliberately

```sh
ctxindex sync --realm company --json
ctxindex status --realm company --json
```
...
```sh
ctxindex search "quarterly planning" --realm company --json
