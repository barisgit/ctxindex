...
```sh
ctxindex sync --source company-mail --format json
ctxindex status --source company-mail --format json
```

Omit `--source` from `sync` or `status` to select all configured Sources. Sync is a strategy for fast local discovery; it is selected by Source, not Realm. Providers and files remain canonical. Purging local materializations does not delete provider records.
