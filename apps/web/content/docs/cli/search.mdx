...
Search accepts query text, filters, or both. With no query, at least one filter is required and the search remains local.
...
| `--remote` | Search remote-capable Sources only. A query is required. |
