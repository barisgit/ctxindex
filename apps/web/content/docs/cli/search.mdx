...
| `--continuation <token>` | Resume one exact remote Source using the opaque token returned in JSON pagination. Requires `--remote`, exactly one `--source`, and the unchanged query and limit. |
...
Local JSON pagination is `{ offset, limit, hasMore }`. One exact remote Source instead returns `{ limit, hasMore, continuation }`; repeat the unchanged remote search and `--limit` with `--continuation` while `hasMore` is true. Multi-Source remote searches do not expose one global continuation.
