...
- The bundled Google and Microsoft OAuth Apps are not verified. The maintainer is an individual and cannot get them verified at the moment; provider review can also take several weeks. See the [provider notes](/docs/start/connect-provider#inspect-available-oauth-apps).
...
ctxindex describe adapter <adapter-id> --format json
```

## Search returns nothing

- Confirm the Source belongs to the Realm you filtered.
- Run `sync --source <label> --format json` for indexed Sources.
