...
- Verification scripts are deterministic command-line gates: they scan production source/config, optionally spawn or import runtime targets, print findings, and signal success or failure by exit status. See `scripts/verify/codemap.md`.
...
2. Verifiers inspect declared production paths and package exports, then return contract-specific status output and exit codes.
...
- Verification targets include `apps/cli`, `packages/core`, `packages/adapters`, package manifests, and selected script sources.
