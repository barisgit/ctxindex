...
- `cli-framework-citty.ts` combines source assertions with spawned CLI smoke checks to enforce Citty command wiring, generated client/account help and version output, and nonzero handling of unknown commands.
...
- `github-actions-ci.test.ts` parses the pull-request workflow and locks its main-branch trigger, read-only permissions, concurrency cancellation, Bun pin, frozen-lockfile install, and repository CI command.
- `package-dependencies.ts` discovers every `apps/*` and `packages/*` workspace from the root manifest, recursively scans JavaScript/TypeScript source with generated/build directories excluded, and uses the TypeScript AST to enforce direct runtime declarations, dependency use, and workspace direction. It reads workspace-local `tsconfig.json` path prefixes so aliases such as the web app's `@/` and generated `collections/` imports remain local, normalizes React JSX runtime imports, ignores Fumadocs' framework-provided `mdx/types`, and accounts for declared framework peer dependencies. `communication-message-profile.test.ts` owns the bundled message Profile-to-registry contract; `calendar-event-profile.integration.test.ts` verifies the provider-neutral calendar Profile through generic registry, storage, search, exact-Realm, retrieval/cache, and relation paths using fake Adapters.
...
- `network-egress.sh` combines literal-host and raw/alternate-client scans with discovery of every production Adapter `context.fetch` caller and its co-located test. It fixes test-only mock endpoint ownership, then runs the production chokepoint, linked-provider context, redaction, no-send, malformed-command zero-side-effect, and CLI egress suites.
...
- Scans `apps/cli/src`, `apps/cli/bin/ctxindex.mjs`, `apps/web`, `packages/core`, `packages/adapters`, and selected `scripts` paths according to each gate.
