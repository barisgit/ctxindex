...
- `cli-thin-lines.ts` enforces an 80-line nonblank, non-import budget on file paths supplied through `process.argv`.
...
3. Findings are printed with file/line context; the script exits `1` on violations (`cli-thin-lines.ts` uses `2` for missing arguments).
