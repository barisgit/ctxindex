...
Gmail reply creation/update uses the existing Draft endpoint and response parser. The complete MIME raw value includes locally CR/LF-validated derived headers, while the JSON `message` object includes `threadId`. Microsoft reply creation posts to the parent message's `createReply` route and normalizes the returned Draft with `replyToRef`; update PATCHes the locally addressed Draft. Microsoft rejects a response whose normalized recipient, subject, body, or conversation differs from the validated reply. Both providers return `RetrievedResource` values which core validates and materializes through the existing complete `adhoc` Action path.
