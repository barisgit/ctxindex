...
#### Scenario: Graph adds an unknown event type
- **WHEN** Graph returns an otherwise-valid event with a non-empty `type` value newer than the Adapter's recognized vocabulary
- **THEN** the type is treated as absent, the event materializes, and occurrence-only fallback behavior is not applied
