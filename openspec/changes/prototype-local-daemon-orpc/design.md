...
### 3. `@ctxindex/rpc` is a separate package with zero business logic

A separate private workspace package declares only exact bounded wire schemas and schema-derived types, safe declared errors, a narrow injected `DaemonRpcApplication` interface, compatibility/cross-cutting middleware, the oRPC router factory, and exported router/client types. Procedure handlers validate input, delegate exactly once to that interface, validate/serialize the returned result, and apply no branch based on business data. `createDaemonRouter(application, expectations)` receives protocol/runtime identity expectations as immutable construction data, so middleware never calls `application.health()` as hidden delegation. The package owns no database, runtime construction, provider access, Extension loading, filesystem/process lifecycle, CLI formatting, Source selection or iteration, retry loop, use-case orchestration, error-class inspection, or domain decision. The daemon application implements the injected interface and owns all use-case orchestration, including calls into core business services and safe DTO projection.
...
The Bun transport adapter lives in the daemon and the socket-aware client adapter lives in the CLI. This preserves the option of a future Node transport adapter without weakening the prototype's Bun-first choice. A contract-first oRPC package is deferred because the protocol is private and both ends are built in one workspace.
...
### 7. Every DTO and error crosses as a closed bounded safe envelope

JavaScript error identity cannot survive RPC. Every request and result uses the exact strict bounded DTOs in `implementation.md`. Procedures return one result envelope and only declared transport-safe failure variants. Per-Source sync failures are projections, never `Error` values. Oversized search, Resource, or thread projections fail as `result_too_large`; they are never truncated into a different valid result. Causes, stacks, diagnostics bags, provider bodies, tokens, Extension paths, raw paths, and raw backend errors never cross the boundary. The CLI reconstructs public classification and remains the only numeric exit mapper. Daemon unavailability, protocol mismatch, runtime mismatch, database-lease conflict, shutdown timeout, prototype-unsupported, and result-too-large all use exit `50`; cancellation retains `130` and locally invalid input retains `2`.

### 8. Unary request cancellation must reach the selected operation

The client attaches its `AbortSignal` to the oRPC call; every router procedure forwards the original server request signal through the injected application interface. Daemon orchestration passes it to the selected Realm, Source, sync, search, retrieval, or thread operation and, where applicable, through core orchestration and Adapters. The e2e proof interrupts a real sync, requires exit `130`, and verifies the daemon records cancellation rather than continuing silently; focused tests prove signal identity for every procedure.
...
None block implementation. Authentication middleware, full command migration, service management, background scheduling, byte streaming, and Node transport are intentionally post-prototype decisions.
