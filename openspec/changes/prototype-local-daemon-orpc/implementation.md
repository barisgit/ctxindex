...
For every migrated command, validated lifecycle/discovery metadata for the exact canonical tuple or a test endpoint override selects daemon routing. Once selected, stale metadata, an unreachable endpoint, or a lost connection returns `daemon_unavailable`; none falls back or opens SQLite in the client. Without selection, commands that retain a direct implementation use it behind the shared-lease fence on Darwin. Before any direct stateful path constructs dependencies, the CLI uses `@ctxindex/local-daemon` to acquire a shared lease for its canonical SQLite path. Exclusive conflict returns `prototype_unsupported` before open even when state roots differ. The CLI retains the shared lease across runtime composition and SQLite use and releases it only after database close. A typed unsupported-platform result skips only this direct shared-lease wrapper because daemon startup cannot acquire ownership there; an unsupported Darwin filesystem/primitive still fails closed.
