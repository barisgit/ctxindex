...
- Add a separate private `@ctxindex/rpc` package containing only exact bounded wire schemas/types, oRPC procedure composition, one narrow injected application interface, and compatibility/cross-cutting middleware; all use-case and business logic remains in the daemon application and core.
