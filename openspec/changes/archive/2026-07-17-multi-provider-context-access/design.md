...
The SDK OAuth auth declaration gains a stable provider id and provider metadata sufficient for a uniform host flow: authorization/token/identity URLs; identity subject, ordered label, and typed Account Identity JSON paths; PKCE/client-auth mode; provider identity/refresh scopes; approved auth-plane hosts; safe default environment variable names; and fixed authorization parameters. Adapter definitions retain their operation scopes and separately bounded provider-API hosts alongside the shared provider descriptor. Registry validation rejects malformed URLs, inconsistent definitions for one provider, unsafe scope tokens, and duplicate/empty scopes.

`auth add <provider> --adapter <id>... (--loopback|--from-env) [--client-id <public-id>] [--label <label>]` requires at least one selected loaded Adapter, all from the requested provider. Core requests the strict sorted union of only those Adapter scopes and provider base scopes. `--loopback` is explicit; there is no TTY-triggered browser launch or deprecated out-of-band code input. State, callback path, timeout, and PKCE verifier/challenge are validated. Long-lived values and client secrets come only from central environment names or typed secret input, never literal argv. `--from-env` is the explicit headless/checkpoint path and validates the token response before identity/storage. When `scope` is present, every selected Adapter operation scope must appear by case-sensitive comparison; provider identity/refresh scopes may be omitted by Microsoft and are instead proven by successful identity/refresh behavior. An absent initial `scope` means the RFC-compatible requested set, while an absent refresh `scope` preserves the Grant's prior normalized set.
