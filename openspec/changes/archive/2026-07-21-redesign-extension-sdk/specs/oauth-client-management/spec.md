...
### Requirement: OAuth Clients are public configuration records
**Reason**: Public Client records are replaced by OAuth App definitions activated from Extensions or local secret-backed BYOA records.

### Requirement: OAuth Client labels may be defaulted
**Reason**: An OAuth App's authored `label` is its required stable Provider-scoped selector.

### Requirement: Client inventory exposes configuration metadata
**Reason**: The unified OAuth App inventory exposes only safe identity and provenance fields.
