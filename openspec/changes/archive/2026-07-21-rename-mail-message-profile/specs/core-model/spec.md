...
#### Scenario: Thread traversal remains Profile-neutral
- **WHEN** Resources from any loaded Profile declare `conversation` or `parent` Relations
- **THEN** generic thread traversal follows those Relations without branching on `mail.message` or any other Profile id
