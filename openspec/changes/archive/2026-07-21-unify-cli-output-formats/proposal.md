...
- Make `--format` the sole output selector, add `-f` as its short alias, and remove the pre-release `--json` compatibility flag.
- Add conservative aliases for frequent unambiguous selectors: `-s` for Source, `-r` for Realm, `-l` for result limit, and `-o` for file output.
...
- Keep sync streaming, Profile export, and describe output as separate format domains while using the same `--format`/`-f` selector grammar.
