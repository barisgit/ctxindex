...
3. The SDK carries an optional opaque continuation from Adapter result to the next Adapter query. Microsoft owns its cursor format and encodes a versioned validated Graph next link, the exact Source id, normalized query identity, requested limit, and previously seen immutable message ids. Binding the token to the exact Source, unchanged query, and requested limit prevents accidental cursor reuse while the Adapter independently caps each invocation at 50 results; bounded seen ids suppress cross-page duplicates without provider-specific core state.
4. Microsoft omits `$search` for match-all enumeration, uses message `$search` only for documented text/sender/time KQL, and uses `$filter=isRead eq false` for Profile `unread=true` or `$filter=isRead eq true` for `unread=false` when no `$search` is needed. Because Graph message search does not document combining `$search` and `$filter`, combined text/KQL plus unread uses `$search` alone and verifies the exact unread boolean locally and again through the core Profile extractor. Draft messages remain excluded after parsing.
