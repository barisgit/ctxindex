...
**Goals:**
- Make absence of the persisted config an explicit, safe precondition failure for database-backed commands.
...
1. The persisted config file is the initialization marker because `init` selects and writes the secret backend before bootstrapping SQLite. A shared CLI preflight checks only file existence and returns fixed guidance; it does not read config, create directories, open SQLite, or initialize a backend.
...
3. The failure is a usage/precondition error with stable exit code 2 and the actionable message `ctxindex is not initialized; run bun cli init`. No native backend detail is exposed because no backend effect occurs.
...
- [A config exists after a partially completed init but SQLite does not] → Database bootstrap remains idempotent once backend selection has been persisted, so later database-backed commands can safely open the missing database.
