...
- Replace the public Client CLI with exact OAuth App commands: `oauth-app add <provider> <label> --from-env`, `oauth-app list [--json]`, and `oauth-app remove <provider> <label>`. `account add <provider> --app <label> [--label <label>]` always requires the exact App label. Provider registration metadata maps typed top-level App config keys to environment variable names solely for local BYOA import; no literal config or secret argv is accepted.
