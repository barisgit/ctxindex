...
- Add one shared `--format pretty|text|json` contract to structured read commands, with `pretty` selected on a TTY and `text` otherwise.
...
- Make text output deterministic and low-token: escaped TSV for collections and a labeled complete envelope with compact nested JSON for singular Resources.
...
- Defer `sync` output migration until its streaming contract lands, while documenting the intended mapping to the shared formats.
