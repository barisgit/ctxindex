...
Structured read commands currently use unrelated booleans and command-specific enums, and their formatters range from one-field lines to fixed tables wider than 250 columns. Humans need a readable default in an interactive terminal, while shell-capable agents need deterministic complete output without paying JSON's repeated-key cost. Existing scripts already rely on `--format json`, and `export --format` plus `describe --format` already use the word “format” for different domain-specific contracts.
...
3. `--format` is the only output selector and `-f` is its Citty alias. The pre-release `--json` flag is removed before it becomes a compatibility obligation. Agent documentation uses the explicit long form `--format json`.
...
8. The shared presentation contract applies to search, get, thread, Artifact list, status, and the five primary inventories. Commands with distinct domains still use the common selector grammar: sync accepts `summary|events|compact|json`, Profile export accepts Profile formats, and describe accepts `text|markdown|json`. Search `--refs` is a text-only projection and rejects explicit pretty or JSON selectors.

## Risks / Trade-offs

- [Pre-release callers still use `--json`] → remove it before `0.1.0`, update every shipped example and test, and keep `--format json` explicit.
...
No persistent or deployed state changes. Pre-release `--json` callers must use `--format json` (or `-f json`). Callers that parse readable output must select `--format text` and adopt escaped TSV or labeled-envelope parsing.
