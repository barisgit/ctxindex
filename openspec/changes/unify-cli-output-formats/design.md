...
4. Pretty rendering uses the existing `cli-table3` dependency through one shared renderer. Wide layouts receive explicit semantic columns; narrow output uses one vertical card per record. Values wrap but are never ellipsized or sliced.
5. Text collections are escaped TSV with a deterministic header. Tabs, newlines, carriage returns, and backslashes are escaped so each logical record occupies one line. Singular Resources use fixed labeled envelope fields followed by compact JSON for the nested profile and payload.
