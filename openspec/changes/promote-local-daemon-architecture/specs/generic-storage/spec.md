...
Every production SQLite opener MUST participate in the same canonical database ownership protocol. The daemon MUST retain exclusive ownership from before open until after close; any allowlisted direct opener MUST retain shared ownership over the same interval. Platform implementations MUST preserve equivalent contention, crash-release, private-file, and fail-closed semantics.
