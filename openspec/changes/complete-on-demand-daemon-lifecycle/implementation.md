...
Command flow is: parse and locally validate, classify through the promoted stateful/safe-exception inventory, verify initialization evidence, ensure ready, create the exact typed daemon selection, then invoke one semantic procedure. Safe pre-initialization surfaces bypass ensure. A declared `daemon_unavailable` rejection before procedure admission may wait for a stopping owner to release and repeat ensure once. Ambiguous transport failures and all admitted procedures are never replayed. All failures normalize through the existing daemon failure registry and CLI exit mapper.
