...
### Requirement: Managed OAuth failures keep stable exits and safe BYOA guidance
...
These failures MUST make no partial Account or Grant change and MUST provide deterministic guidance to run `oauth-app add <provider> <label> --from-env` followed by `account add <provider> --app <label>`. Once provider egress begins, ctxindex MUST NOT automatically retry through another App. Guidance, errors, logs, diagnostics, inventory, persistence, and provider-response output MUST NOT expose App config, environment values, client ids, secret references/values, tokens, authorization codes, PKCE verifiers, raw provider responses, private identities, or state outside the one dedicated authorization-URL delivery surface.
...
- **THEN** CLI exits `50`, emits only the safe typed category and static fallback commands, and persists no Account or Grant

#### Scenario: Permission denial keeps permission exit
- **WHEN** a provider reports typed `permission_denied`
- **THEN** CLI exits `40`, emits only the safe typed category and static fallback commands, and persists no Account or Grant
