...
7. **Failure never starts a second flow.** Missing/default-policy and managed-App resolution failures happen before secret reads, persistence, browser launch, or provider egress, exit as invalid usage, and include explicit BYOA commands. Once managed App resolution succeeds, provider rejection, quota, network, or permission failure retains its existing stable category, persists no partial Account/Grant, and receives no selection fallback. No automatic credential switch or second browser is opened.
