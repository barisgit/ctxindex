...
Keychain tests use explicit deferred gates to deterministically interleave concurrent writes across backend instances, verify complete deterministic inventory, cover inventory/credential failure ordering and retryable deletion, and force probe read/deletion failures without native Keychain access. Authentication tests use explicit write gates to overlap same-Account reauthorization and refresh, require that only committed Grant refs remain authoritative, force cleanup deletion failures, assert committed replacement/removal state remains usable, verify idempotent physical cleanup retry, and inspect warnings for bounded safe fields and absence of canaries/refs.
