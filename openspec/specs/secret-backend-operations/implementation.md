...
Keychain writes, deletes, inventory reads, and availability probes share one process-wide asynchronous critical section across backend instances. A write publishes its intended reserved-index entry before persisting the credential value and attempts to restore the prior index when the credential write reports failure. If restoration also reports failure, the original credential-write error remains authoritative and the intended inventory entry may remain; no reference or success is returned, and the adapter does not infer whether a native call that reported failure took effect. A delete removes the credential before its index entry so incomplete work remains discoverable and retryable. The probe reuses one reserved credential identity in a service namespace outside normal `ctxindex/<scope>` services, always attempts deletion after a successful write, and treats read or deletion failure as unavailable so the next probe retries the same row without risking a scoped-secret collision. The queue always releases after failure. This is an in-process consistency boundary, not a cross-process lock or a new Keychain storage format.
