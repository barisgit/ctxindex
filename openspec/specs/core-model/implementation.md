...
Candidate construction validates complete selected graphs without mutating active state. OAuth App duplicates always conflict. Exact reused non-App objects may coalesce. Distinct same-identity values containing functions or Zod schemas conflict. Distinct pure declarative values may coalesce only when canonical structural equality proves equality. Provenance is retained only for diagnostics.
