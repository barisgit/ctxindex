...
Gmail Draft identity uses the immutable Draft id; Outlook requests immutable Graph ids. Portable strict union inputs keep standalone content replacement unchanged while reply branches accept only local parent Ref and body text. Adapters derive reply recipient, subject, and threading state locally before one provider mutation; Gmail rejects CR/LF in every derived MIME header value. The CLI recursively renders each strict union branch and exposes only registry-derived `action describe` and `action run`; no provider-specific parallel command family or send route exists.
