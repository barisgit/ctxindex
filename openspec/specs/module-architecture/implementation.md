...
@ctxindex/extension-sdk
  ...
  no reference factories, dependency graph, host callback, registration, or docs fields
```
...
Use Bun's colocated unit/integration/e2e tests. Storage tests create fresh sandboxes; provider tests use loopback-only authorized HTTP. `scripts/verify/architecture-lint.ts`, package-dependency checks, SDK inference fixtures, common-origin activation tests, and relocated compiled-host and CLI tests enforce this shape. Verification rejects reference/dependency/host-callback/docs surface, providerless authorization fields, origin-specific registration, and pre-validation registry mutation.
