...
Authorization, refresh, and removal use one process-wide asynchronous queue keyed by exact Provider and external user id. Each operation re-reads current Grant state after entering the Account critical section, so same-Account replacements clean the state they actually supersede while unrelated Accounts remain concurrent.
