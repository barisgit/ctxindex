...
After managed selection, Provider and authorization failures retain their existing typed categories and exits; selection does not reinterpret them to obtain a fallback. CLI formatting appends only static `oauth-app add <provider> <label> --from-env` and `account add <provider> --app <label>` guidance. Missing managed policy uses the same safe commands with `invalid_oauth_selection`. Neither path starts another authorization or exposes App config, client ids, environment values, secret references, tokens, codes, verifiers, raw Provider responses, private identities, or state outside the dedicated authorization-URL surface.
